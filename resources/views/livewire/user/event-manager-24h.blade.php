...
<div>
        ...
        <ul class="flex flex-wrap -mb-px text-sm font-medium text-center text-gray-500 dark:text-gray-400">
            <li class="me-2">
                <flux:button wire:click="setTab('added-events')" 
                             class="inline-flex items-center justify-center p-4 border-b-2 {{ $activeTab === 'added-events' ? 'text-blue-600 border-blue-600 dark:text-blue-500 dark:border-blue-500' : 'border-transparent hover:text-gray-600 hover:border-gray-300 dark:hover:text-gray-300' }} rounded-t-lg group"
                             icon="plus-circle"
                             variant="ghost">
                    Evenimente Adăugate
                </flux:button>
            ...
            <li class="me-2">
                <flux:button wire:click="setTab('all-events')" 
                             class="inline-flex items-center justify-center p-4 border-b-2 {{ $activeTab === 'all-events' ? 'text-blue-600 border-blue-600 dark:text-blue-500 dark:border-blue-500' : 'border-transparent hover:text-gray-600 hover:border-gray-300 dark:hover:text-gray-300' }} rounded-t-lg group"
                             icon="list-bullet"
                             variant="ghost">
            ...
            <div>
                <div class="mb-4">
                    @can('create events 24h')
                        <button wire:click="$set('showModal', true)" class="bg-blue-500 hover:bg-blue-600 text-white px-4 py-2 rounded">
                            Adaugă eveniment
                        </button>
                    @endcan
                </div>
                <div class="overflow-y-auto max-h-[500px]" x-data="{ loading: false }" 
                    ...
                    @forelse ($events->sortKeysDesc() as $date => $dayEvents)
                        ...
                        <div class="mb-4">
                            <table class="w-full bg-gray-100 dark:bg-gray-800 border border-gray-300 dark:border-gray-700 text-sm">
                                <thead>
                                    <tr>
                                        <th class="py-2 px-4 border-b border-r border-gray-300 dark:border-gray-700 text-gray-900 dark:text-white text-left">Data evenimentului</th>
                                        <th class="py-2 px-4 border-b border-r border-gray-300 dark:border-gray-700 text-gray-900 dark:text-white text-left">Categoria</th>
                                        <th class="py-2 px-4 border-b border-gray-300 dark:border-gray-700 text-gray-900 dark:text-white text-left">Conținut</th>
                                        <th class="py-2 px-4 border-b border-gray-300 dark:border-gray-700 text-gray-900 dark:text-white text-center">Acțiuni</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    @php
                                        ...
                                        @foreach ($categoryEvents as $index => $event)
                                            <tr>
                                                @if ($index === 0 && $categoryEvents === $groupedByCategory->first())
                                                    <td class="py-2 px-4 border-b border-r border-gray-300 dark:border-gray-700 text-gray-900 dark:text-white text-left align-top" rowspan="{{ $totalEventCount }}">
                                                        {{ Carbon\Carbon::parse($event->data)->format('d.m.Y') }}
                                                    </td>
                                                @endif
                                                @if ($index === 0)
                                                    <td class="py-2 px-4 border-b border-r border-gray-300 dark:border-gray-700 text-gray-900 dark:text-white text-left align-top" rowspan="{{ $categoryEventCount }}">
                                                        {{ $categoryName }}
                                                    </td>
                                                @endif
                                                <td class="py-2 px-4 border-b border-gray-300 dark:border-gray-700 text-gray-900 dark:text-white text-left">
                                                    {!! $event->events_text ?? '-' !!}
                                                </td>
        ...
        @elseif ($activeTab === 'all-events')
            <div>
                <div class="mb-4 flex justify-between items-center">
                    <div>
                        ...
                        <input type="date" id="filterDate" wire:model="filterDate" wire:change="updateFilterDate"
                               class="border border-gray-300 dark:border-gray-700 bg-gray-100 dark:bg-gray-900 text-gray-900 dark:text-white p-2 rounded text-sm">
                    </div>
                    <div>
                ...
                <div class="overflow-x-auto">
                    <table id="all-events-table" class="w-full bg-gray-100 dark:bg-gray-800 border border-gray-300 dark:border-gray-700 text-sm">
                        <thead>
                            <tr>
                                <th class="py-2 px-4 border-b border-gray-300 dark:border-gray-700 text-gray-900 dark:text-white text-left">Categoria</th>
                                <th class="py-2 px-4 border-b border-gray-300 dark:border-gray-700 text-gray-900 dark:text-white text-left">Instituția</th>
                                <th class="py-2 px-4 border-b border-gray-300 dark:border-gray-700 text-gray-900 dark:text-white text-left">Conținut</th>
                            </tr>
                        </thead>
                        <tbody>
                            @forelse ($allEvents as $categoryData)
                                <tr>
                                    <td class="py-2 px-4 border-b border-r border-gray-300 dark:border-gray-700 text-gray-900 dark:text-white text-left align-top" rowspan="{{ $categoryData['events']->count() }}">
    ...
    @if ($showModal)
        <div class="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center" wire:ignore.self>
            <div class="bg-gray-100 dark:bg-gray-800 p-4 rounded-lg w-1/3 text-gray-900 dark:text-white border border-gray-300 dark:border-gray-700">
                ...
                <form wire:submit.prevent="{{ $editingEventId ? 'updateEvent' : 'createEvent' }}">
                        ...
                        <div class="flex-1 w-1/4">
                            ...
                            @can('schimbare data evenimente')
                                <input type="date" wire:model="data" 
                                       class="w-full border border-gray-300 dark:border-gray-700 bg-gray-100 dark:bg-gray-900 text-gray-900 dark:text-white p-2 rounded focus:outline-none focus:ring-2 focus:ring-blue-500 text-sm">
                            @else
                                <input type="text" value="{{ Carbon\Carbon::today()->format('Y-m-d') }}" readonly
                                       class="w-full border border-gray-300 dark:border-gray-700 bg-gray-100 dark:bg-gray-900 text-gray-900 dark:text-white p-2 rounded text-sm">
                            @endcan
                    ...
                    <div class="mb-3">
                        ...
                        <textarea wire:model="events_text" 
                                  class="w-full border border-gray-300 dark:border-gray-700 bg-gray-100 dark:bg-gray-900 text-gray-900 dark:text-white p-2 rounded focus:outline-none focus:ring-2 focus:ring-blue-500 min-h-[150px] text-sm"></textarea>
